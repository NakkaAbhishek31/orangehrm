...
- Name: Performance\performance.spec.ts >> Performance - Employee Reviews >> TC_PERFORMANCE_211 - Selected sub unit should remain after searching reviews @positive @filter @persistence @regression
- Location: tests\Performance\performance.spec.ts:480:5
...
Locator: locator('form').locator('.oxd-input-group').filter({ has: locator('label').getByText('Sub Unit', { exact: true }) }).locator('.oxd-select-text')
...
Call log:
  ...
  - waiting for locator('form').locator('.oxd-input-group').filter({ has: locator('label').getByText('Sub Unit', { exact: true }) }).locator('.oxd-select-text')
