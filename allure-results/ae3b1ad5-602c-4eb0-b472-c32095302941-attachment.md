...
Locator: getByRole('dialog').getByRole('button', { name: 'Yes, Delete', exact: true })
Expected: visible
Timeout: 20000ms
  ...
  - rowgroup:
    ...
    - row " ess1786597481977159 ESS Auto1786597481977159 User Enabled  ":
      ...
      - cell " ":
        - button ""
        - button ""
    - row " ess1786598667561936 ESS Auto1786598667561936 User Enabled  ":
      - cell "":
        - checkbox ""
        - text: 
      - cell "ess1786598667561936"
      - cell "ESS"
      - cell "Auto1786598667561936 User"
      - cell "Enabled"
      - cell " ":
        - button ""
        - button ""
    ...
    - row " essA1786598598555249 ESS AutoA1786598598555249 User Enabled  ":
      ...
      - cell " ":
        - button ""
        - button ""
    - row " essA1786598784976276 ESS AutoA1786598784976276 User Enabled  ":
      - cell "":
        - checkbox "" [checked]
        - text: 
      - cell "essA1786598784976276"
      - cell "ESS"
      - cell "AutoA1786598784976276 User"
      - cell "Enabled"
      - cell " ":
        - button ""
        - button ""
    ...
    - row " essB1786598598555249 ESS AutoB1786598598555249 User Enabled  ":
      ...
      - cell " ":
        - button ""
        - button ""
    - row " essB1786598784976276 ESS AutoB1786598784976276 User Enabled  ":
      - cell "":
        - checkbox "" [checked]
        - text: 
      - cell "essB1786598784976276"
      - cell "ESS"
      - cell "AutoB1786598784976276 User"
      - cell "Enabled"
      - cell " ":
        - button ""
        - button ""
    ...
    - row " pimuser98585085 ESS Raj ravi Enabled  ":
      ...
      - cell " ":
        - button ""
        - button ""
    - row " pimuser98786070 ESS Raj ravi Enabled  ":
      - cell "":
        - checkbox ""
        - text: 
      - cell "pimuser98786070"
      - cell "ESS"
      - cell "Raj ravi"
      - cell "Enabled"
      - cell " ":
...
```ts
  ...
  757 | 
  758 |     const confirmDeleteButton = confirmationDialog.getByRole("button", {
  759 |       name: "Yes, Delete",
  760 |       exact: true,
  761 |     });
  762 | 
> 763 |     await expect(confirmDeleteButton).toBeVisible();
      |                                       ^ Error: expect(locator).toBeVisible() failed
  764 |     await expect(confirmDeleteButton).toBeEnabled();
  765 | 
  766 |     const deleteResponse = this.page.waitForResponse(
  767 |       (response) =>
  768 |         response.url().includes("/api/v2/admin/users") &&
  769 |         response.request().method() === "DELETE" &&
  770 |         response.ok(),
  771 |       { timeout: 20_000 },
  772 |     );
  773 | 
  774 |     await confirmDeleteButton.click();
  775 |     await deleteResponse;
  776 | 
  777 |     await expect(confirmationDialog).toBeHidden({
  778 |       timeout: 20_000,
  779 |     });
  780 | 
  781 |     await expect(
  782 |       this.toastMessage.filter({
  783 |         hasText: /Successfully Deleted/i,
  784 |       }),
  785 |     ).toBeVisible({
  786 |       timeout: 20_000,
  787 |     });
  788 | 
  789 |     await expect(this.loadingSpinner).toBeHidden();
  790 |   }
  791 | 
  792 |   async updateSystemUsername(updatedUsername: string): Promise<void> {
  793 |     await this.addUsernameInput.clear();
  794 |     await this.addUsernameInput.fill(updatedUsername);
  795 | 
  796 |     await this.addUsernameInput.blur();
  797 | 
  798 |     await expect(this.addUsernameInput).toHaveValue(updatedUsername);
  799 | 
  800 |     await expect(this.usernameValidation).toBeHidden();
  801 | 
  802 |     const updateUserResponse = this.page.waitForResponse(
  803 |       (response) =>
  804 |         /\/api\/v2\/admin\/users\/\d+$/.test(response.url()) &&
  805 |         response.request().method() === "PUT" &&
  806 |         response.ok(),
  807 |       { timeout: 20_000 },
  808 |     );
  809 | 
  810 |     await this.editUserSaveButton.click();
  811 |     await updateUserResponse;
  812 | 
  813 |     await expect(this.page).toHaveURL(/admin\/viewSystemUsers/, {
  814 |       timeout: 20_000,
  815 |     });
  816 |     await expect(this.systemUsersHeading).toBeVisible();
  817 |   }
  818 | }
  819 | 
```