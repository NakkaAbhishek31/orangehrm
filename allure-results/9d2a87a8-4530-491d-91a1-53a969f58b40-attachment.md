...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_105 - Admin should find a newly created employee by ID @positive @regression
- Location: tests\Admin\admin-users.spec.ts:1621:7
...
Call log:
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ hasText: '0603' }).locator('.oxd-table-cell').nth(2)
