...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_094 - New employee ID should replace previous employee ID filter @positive @filter @regression
- Location: tests\Admin\admin-users.spec.ts:1367:7
...
Call log:
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ hasText: '0596' }).locator('.oxd-table-cell').nth(2)
