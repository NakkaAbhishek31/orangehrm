...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_088 - Admin should open employee details after searching by employee ID @positive @navigation @regression
- Location: tests\Admin\admin-users.spec.ts:1204:7
...
Locator:  locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).filter({ hasText: '0591' }) })
Expected: 1
Received: 0
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).filter({ hasText: '0591' }) })
