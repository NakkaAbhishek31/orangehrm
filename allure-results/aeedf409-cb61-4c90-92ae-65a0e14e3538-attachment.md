...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_084 - Admin should open employee details after cancelling deletion @negative @delete @navigation @regression
- Location: tests\Admin\admin-users.spec.ts:1114:7
...
Locator:  locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).getByText('0572', { exact: true }) })
Expected: 1
Received: 0
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).getByText('0572', { exact: true }) })
