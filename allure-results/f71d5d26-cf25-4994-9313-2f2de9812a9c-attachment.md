...
- textbox "First Name": Auto1786380888793632
...
> 54  |     await expect(pimPage.personalDetailsHeading).toBeVisible();
  ...
  133 |     await navigationPage.gotoPIM(); await pimPage.gotoAddEmployee();
  134 |     await pimPage.fillAddEmployeeForm(employee);
  135 |     await pimPage.createLoginDetailsSwitch.click();
  136 |     await pimPage.employeeUsernameInput.fill(`${data.login.usernamePrefix}${unique()}`);
  137 |     await pimPage.employeePasswordInput.fill(data.login.password);
  138 |     await pimPage.confirmPasswordInput.fill(data.login.confirmPassword);
  139 |     await pimPage.confirmPasswordInput.blur();
  140 |     await expect(pimPage.confirmPasswordValidation).toHaveText(data.expectedError);
  141 |     await pimPage.SaveEmployeeButton.click();
  142 |     await expect(page).toHaveURL(/pim\/addEmployee/);
  143 |   });
  144 | 
  145 |   test("TC_PIM_032 - Weak password validation should appear @negative @validation @account @regression", async ({ page, navigationPage, pimPage }) => {
  146 |     const data = pimData.TC_PIM_032;
  147 |     await navigationPage.gotoPIM(); await pimPage.gotoAddEmployee();
  148 |     await pimPage.fillAddEmployeeForm(employeeFrom(data.employee));
  149 |     await pimPage.createLoginDetailsSwitch.click();
  150 |     await pimPage.employeeUsernameInput.fill(`${data.login.usernamePrefix}${unique()}`);
  151 |     await pimPage.employeePasswordInput.fill(data.login.password);
  152 |     await pimPage.confirmPasswordInput.fill(data.login.password);
  153 |     await pimPage.confirmPasswordInput.blur();
  154 |     await expect(pimPage.passwordValidation).toContainText(new RegExp(data.expectedPasswordPattern, "i"));
```