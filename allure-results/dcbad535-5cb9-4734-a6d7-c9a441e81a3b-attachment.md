...
- generic [ref=f5e3]:
  ...
  - generic [ref=f5e148]:
    - generic [ref=f5e149]:
      ...
      - generic [ref=f5e184]:
        - generic [ref=f5e185]: (127) Records Found
        - table [ref=f5e189]:
          - rowgroup [ref=f5e190]:
            - row [ref=f5e191]:
...
```ts
  ...
  347 |      await expect(
  348 |       timePage.attendanceHeading
  349 |     ).toBeVisible();
  350 | 
  351 |     await expect(
  352 |       timePage.attendanceEmployeeInput
  353 |     ).toBeVisible();
  354 |     // Leave Employee Name empty.
  355 |       await timePage.attendanceEmployeeInput.fill('');
  356 |     // Keep the default attendance date.
  357 |      const defaultDate =
  358 |       await timePage
  359 |         .attendanceDateInput
  360 |         .inputValue();
  361 | 
  362 |     expect(defaultDate).not.toBe('');
  363 | 
  364 |     // Click View.
> 365 |      await timePage.attendanceViewButton.click();
      |                                          ^ Error: locator.click: Error: strict mode violation: getByRole('button', { name: 'View', exact: true }) resolved to 51 elements:
  366 |     // Verify Required validation appears
  367 |      await expect(
  368 |       timePage.attendanceEmployeeValidation
  369 |     ).toBeVisible();
  370 | 
  371 |     // below Employee Name.
  372 |     await expect(timePage.attendanceEmployeeValidation).toHaveText(data.requiredMessage);
  373 |     // Verify Employee Name remains empty.
  374 |      await expect(timePage.attendanceEmployeeInput).toHaveValue('');
  375 |     // Verify the user remains on the
  376 |     await expect(page).toHaveURL(
  377 |       new RegExp(
  378 |         `${data.expectedUrlPath}$`
  379 |       )
  380 |     );
  381 | 
  382 |     // Invalid submission must not load records.
  383 |     await expect(
  384 |       timePage.attendanceRows
  385 |     ).toHaveCount(0);
  386 |     // Verify no attendance records are loaded.
  387 |   }
  388 | );
  389 | 
  390 | 
