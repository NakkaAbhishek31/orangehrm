...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_103 - Search filters should retain values after clicking Search @positive @filter @regression
- Location: tests\Admin\admin-users.spec.ts:1571:7
...
Locator: locator('.oxd-autocomplete-option').filter({ hasText: 'Retain1786454518017388' }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-autocomplete-option').filter({ hasText: 'Retain1786454518017388' }).first()
...
- separator
- text: Employee Name
- textbox "Type for hints...": Retain1786454518017388
- listbox:
