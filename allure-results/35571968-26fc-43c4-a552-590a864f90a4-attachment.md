...
- Location: tests\Admin\admin-users.spec.ts:906:7
...
Locator:  locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).getByText('0543', { exact: true }) })
Expected: 1
Received: 0
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).getByText('0543', { exact: true }) })
