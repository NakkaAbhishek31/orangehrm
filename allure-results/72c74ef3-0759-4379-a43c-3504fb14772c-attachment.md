...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_100 - Admin should return to Employee List after opening employee details @positive @navigation @regression
- Location: tests\Admin\admin-users.spec.ts:1504:7
...
Locator:  locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).filter({ hasText: '0599' }) })
Expected: 1
Received: 0
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ has: locator('.oxd-table-cell').nth(1).filter({ hasText: '0599' }) })
