...
- Location: tests\PIM\pim-employee.spec.ts:580:6
...
```ts
  ...
  578 | });
  579 | 
  580 | test.only("TC_PIM_035 - Disabled employee account should not be able to login @negative @security @account @regression", async ({
  581 |   page,
  582 |   loginPage,
  583 |   navigationPage,
  584 |   dashboardPage,
...
> 626 |   await expect(loginPage.errorMessage).toBeVisible();
      |                                        ^ Error: expect(locator).toBeVisible() failed
  627 | 
  628 |   await expect(loginPage.errorMessage).toHaveText("Invalid credentials");
  629 | 
