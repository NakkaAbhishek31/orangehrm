...
- Name: Admin\admin-users.spec.ts >> Admin - System Users >> TC_PIM_081 - Employee details should contain values entered during creation @positive @details @regression
- Location: tests\Admin\admin-users.spec.ts:1052:7
...
Locator:  locator('.oxd-table-body .oxd-table-card').filter({ hasText: '0568' })
Expected: 1
Received: 0
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-table-body .oxd-table-card').filter({ hasText: '0568' })
