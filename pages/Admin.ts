...
readonly usernameValidation:Locator;
  constructor(page: Page) {
    ...
    this.usersMenuItem = page.locator(".oxd-topbar-body-nav-tab-link").filter({
     ...
     this.systemUsersHeading = page.getByRole(
    'heading',
    {
      name: 'System Users',
      exact: true,
  ...
  this.systemUsersTable = page.locator(
    '.oxd-table'
  );

  this.userRows = page.locator(
  ...
  this.addUserHeading = page.getByRole(
  'heading',
  {
    name: 'Add User',
    exact: true,
...
this.addUserSaveButton = page.getByRole(
  'button',
  {
    name: 'Save',
    exact: true,
...
this.addUserCancelButton = page.getByRole(
  'button',
  {
    name: 'Cancel',
    exact: true,
...
async verifySystemUserResult(
  expected: SystemUserFilters
): Promise<void> {
...
async fillSystemUserForm(
  details: AddSystemUserDetails
): Promise<void> {
  await this.selectDropdownOption(
    this.addUserRoleDropdown,
    details.userRole
  );

  await this.addEmployeeNameInput.fill(
  ...
  await this.addPasswordInput.fill(
    details.password
  );

  await this.addConfirmPasswordInput.fill(
    details.password
  );
}
...
async saveSystemUser(): Promise<void> {
  ...
  const successToastAppears = expect(
    this.successToast
  ).toContainText(
    'Successfully Saved',
    {
...
async deleteSystemUserByUsername(
  username: string
): Promise<void> {
